### Essential
- Essential Enterprise Architecture tool https://enterprise-architecture.org/
- https://enterprise-architecture.org/university/essential-meta-model-overview/
- Essential Meta Model https://metamodel.enterprise-architecture.org/metamodel/EA_Class.html ; https://enterprise-architecture.org/university/essential-meta-model-overview/
- The Architecture Elements (Essential Playbook) https://www2.enterprise-architecture.org/architecture_elements.php 
- https://enterprise-architecture.org/resources/free-ea-models/ Загрузите и импортируйте наши модели EA в Essential

#### demo
- https://essentialviewer.com/demo/report ; [form](https://essentialviewer.com/demo/report?XML=reportXML.xml&PMA=essential_baseline_v5_Class83&XSL=common/portal_template.xsl&LABEL=General%20Enterprise%20Architecture%20Portal&cl=en-gb)

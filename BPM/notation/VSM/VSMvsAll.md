...
2. Рассчитайте СКО: Используйте формулу стандартного отклонения:

\(\sigma = \sqrt{\frac{\sum (X_i - \overline{X})^2}{n - 1}}\)где \(X_{i}\) — замер, \(\overline{X}\) — среднее время, а n — количество замеров.

2.1  
Для оценки стабильности времени цикла ($CT$) на конкретном этапе потока используется формула выборочного стандартного отклонения (СКО):

$$\sigma = \sqrt{\frac{\sum_{i=1}^{n} (X_i - \bar{X})^2}{n - 1}}$$

#### Где:
* $\sigma$ (сигма) — среднеквадратичное отклонение.
* $X_i$ — время выполнения процесса в отдельном замере ($i$-й замер).
* $\bar{X}$ — среднее арифметическое времени цикла по всем замерам.
* $n$ — общее количество сделанных замеров.
* $n - 1$ — число степеней свободы (поправка Бесселя для выборки).

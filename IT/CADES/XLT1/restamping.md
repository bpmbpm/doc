...
- Истечение личного сертификата не приводит к немедленной потере юридической силы, если в подписи есть действительные на момент подписания OCSP-ответы и временные метки, подтверждающие, что подпись была создана в период действия сертификата.

#### Визуализация жизненного цикла подписи:

| Событие                                 | Юридическая значимость подписи         |
|------------------------------------------|----------------------------------------|
| Истёк личный сертификат                  | Сохраняется (при наличии действующего TSP)     |
| Истёк сертификат OCSP                 | Сохраняется (при наличии действующего TSP)     |
...
**Вывод:**  
Для долгосрочной проверки (LTV) важно, чтобы OCSP-ответ был корректно подписан и действителен на момент подписания. Его сертификат может быть просрочен на момент перештамповки.
...
### Условия, при которых возможна перештамповка CAdES-XL

Перештамповка не всегда возможна по требованию. Для ее успешного выполнения должны быть соблюдены следующие **обязательные условия**:

1.  **Наличие в исходной подписи CAdES-XL метки времени (RFC 3161).**
    *   Это ключевое условие. Подпись CAdES-XL уже содержит в себе метку времени, которая привязывает момент подписания к доверенному источнику. Эта метка является "якорем" для дальнейшей перештамповки.

2.  **Доступ к полным исходным данным подписи.**
    *   Процесс перештамповки требует не только самой файла подписи (.sig, .p7s), но и исходного подписываемого файла, а также всей цепочки сертификатов (включая корневой УЦ и УЦ метки времени), которые были актуальны на момент создания исходной подписи. Эти данные обычно уже встроены в контейнер CAdES-XL.

3.  **Валидность цепочки сертификатов на момент перештамповки.**
    *   Хотя сертификат подписанта уже мог истечь, цепочка доверия к нему и к сертификату Удостоверяющего Центра (УЦ), выпустившего метку времени, должна быть все еще действительной и доверенной. То есть корневые сертификаты этих УЦ должны присутствовать в хранилище доверенных корневых центров сертификации проверяющей стороны.

4.  **Криптографическая стойкость исходной подписи.**
    *   Алгоритм подписи (например, RSA, ГОСТ Р 34.10-2012) не должен быть на момент перештамповки признан скомпрометированным или слабым. Если алгоритм взломан, перештамповка не придаст подписи новой силы.

### В какой выходной формат происходит перештамповка?

Основной и целевой выходной формат при перештамповке CAdES-XL — это **CAdES-A** (Advanced Signature — расширенная подпись).

*   **CAdES-A** — это финальная, архивная форма подписи, которая содержит в себе все необходимые данные для проверки в будущем, независимо от срока действия сертификатов.
*   **Что добавляется при создании CAdES-A из CAdES-XL:**
    *   **Новая, свежая метка времени (RFC 3161)**, полученная от доверенного TSA (Authority Time Stamp), применяется ко всей подписи в целом, включая старые атрибуты и старую метку времени. Это обновляет "момент доверия" к подписи.
    *   **Архивные атрибуты сертификатов и статусов отзыва (CRL/OCSP-ответы).** В подпись явно встраиваются все цепочки сертификатов и актуальные на момент перештамповки ответы о статусе отзыва для каждого сертификата в цепочке. Это фиксирует их валидность на данный момент.

**Кратко о эволюции форматов CAdES:**
*   **CAdES-B-B** (Base): Подпись с хэш-значением документа.
*   **CAdES-T** (Trust): Добавляет метку времени к хэш-значению.
*   **CAdES-C** (Complete): Добавляет полные сертификаты и информацию о статусе отзыва.
*   **CAdES-X-Long** (XL): Добавляет данные для проверки цепочки доверия (сертификаты и статусы отзыва).
*   **CAdES-A** (Archival): Финализирует процесс, добавляя новую метку времени и архивные данные, делая подпись независимой от внешних источников данных.

### Итог

| Условие для перештамповки | Выходной формат |
| :--- | :--- |
| 1. Исходная подпись имеет формат CAdES-XL (с меткой времени). <br> 2. Есть доступ к исходным данным подписи и документу. <br> 3. Цепочки сертификатов все еще доверенны. <br> 4. Алгоритм подписи криптостоек. | **CAdES-A** (Расширенная/Архивная подпись) |

**Простыми словами:** Перештамповка — это как нотариальное заверение копии уже существующего документа. У вас на руках должен быть оригинальный, ранее заверенный документ (CAdES-XL). Вы приходите к нотариусу (программе для перештамповки), который проверяет подлинность исходного заверения и ставит на вашу копию новую, текущую печать и фиксирует все обстоятельства (CAdES-A), чтобы эту копию можно было принимать и через 10 лет.

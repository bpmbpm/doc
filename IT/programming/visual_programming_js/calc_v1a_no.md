...
<head>
    ...
    <!-- Используем старую версию 3.4.0, где BpmnModeler доступен глобально -->
    <link rel="stylesheet" href="https://unpkg.com/bpmn-js@3.4.0/dist/assets/diagram-js.css">
    <link rel="stylesheet" href="https://unpkg.com/bpmn-js@3.4.0/dist/assets/bpmn-font/css/bpmn.css">
    <style>
...
<script src="https://unpkg.com/bpmn-js@3.4.0/dist/bpmn-modeler.development.js"></script>
<script>
    ...
    log('Страница загружена, версия bpmn-js 3.4.0');

    // Проверяем наличие глобальной переменной
    if (typeof BpmnModeler === 'undefined') {
        log('BpmnModeler не определён! Проверьте загрузку скрипта.', true);
        // Выводим все глобальные объекты, начинающиеся с 'Bpmn' для диагностики
        const bpmnGlobals = Object.keys(window).filter(k => k.toLowerCase().startsWith('bpmn'));
        if (bpmnGlobals.length) {
            log(`Найдены глобальные объекты BPMN: ${bpmnGlobals.join(', ')}`);
        } else {
            log('Не найдено ни одного глобального объекта, начинающегося с "Bpmn"', true);
        }
    } else {
    ...
    if (!canvasDiv) {
        ...
        // BPMN-диаграмма с задачей CalculateTask

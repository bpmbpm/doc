...
## 1. Марковская цепь для критерия «хотя бы один CPU»

При критерии отказа «работоспособность хотя бы одного CPU при условии, что при отказе других не будет потери данных» система находится в работоспособном состоянии до тех пор, пока **хотя бы один CPU** выполняет свои функции. Это классическая модель **параллельного резервирования** (1-out-of-n).

### Марковская цепь (Mermaid)

```mermaid
stateDiagram-v2
    direction LR

    [*] --> S16: Система запущена\n(16 CPU)

    state "S16: 16 CPU (все исправны)" as S16
    state "S15: 15 CPU" as S15
    state "S14: 14 CPU" as S14
    state "S2: 2 CPU" as S2
    state "S1: 1 CPU (последний)" as S1
    state "S0: 0 CPU — ОТКАЗ" as S0

    S16 --> S15: λ_CPU (отказ 1 CPU)
    S15 --> S14: λ_CPU (отказ 2-го CPU)
    S14 --> S2: λ_CPU (отказ 3–14 CPU)
    S2 --> S1: λ_CPU (отказ 15-го CPU)
    S1 --> S0: λ_CPU (отказ последнего CPU)

    S15 --> S16: μ_CPU (восстановление CPU)
    S14 --> S15: μ_CPU (восстановление CPU)
    S2 --> S14: μ_CPU (восстановление CPU)
    S1 --> S2: μ_CPU (восстановление CPU)
    S0 --> S1: μ_CPU (восстановление CPU)

    note right of S1: Система ещё работоспособна\n(деградация ~94%)
    note right of S0: Полный отказ системы\n(потеря данных)
```
...
|:---|:---|:---|
| **S16 – S1** | От 16 до 1 работающего CPU | **Работоспособное** (деградация) |
| **S0** | 0 работающих CPU | **ОТКАЗ** (поглощающее состояние) |

**Ключевой момент:** Состояние S0 является **поглощающим** (absorbing state) — переход из него обратно в работоспособное состояние **невозможен** без перезагрузки, поскольку при отказе последнего CPU система теряет способность выполнять вычисления.

Это соответствует модели, описанной в учебных материалах: «*the reliability R(t) is the probability that the system never enters state 0 at any time during [0,t]... we must modify the Markov chain slightly by removing the transition out of state 0, so that state 0 becomes an absorbing state*».

**Перевод:** «Надёжность R(t) — это вероятность того, что система никогда не перейдёт в состояние 0 в течение [0,t]... необходимо модифицировать марковскую цепь, удалив переход из состояния 0, чтобы состояние 0 стало поглощающим.»

Аналогичная модель используется в работе Trivedi et al.: «*if at least one processor is needed for the system to be up (parallel redundancy), then system reliability, availability and the mean time to failure are all monotonically increasing with the number of processors*».

**Перевод:** «Если для работоспособности системы достаточно хотя бы одного процессора (параллельное резервирование), то надёжность, готовность и среднее время до отказа системы монотонно возрастают с увеличением числа процессоров.»

---

## 2. Чем изоляция отличается от резервирования?

С учётом того, что **восстановление CPU/CMU возможно без остановки сервера** (hot-swap), различие между изоляцией и резервированием становится **фундаментальным** с позиции расчёта надёжности.

### Определения

**Резервирование** — это метод повышения надёжности путём введения **запасных (резервных) элементов**, которые являются **избыточными** по отношению к минимальной структуре системы.

> **Оригинал:** *"Redundancy is expressed in terms of reliability blocks by paralleling blocks... The most common kinds of redundancy are duplication (1-out-of-2 or 1/2)"*
>
> **Перевод:** «Резервирование выражается в терминах блоков надёжности путём параллельного соединения блоков... Наиболее распространённые виды резервирования — дублирование (1 из 2).»

**Изоляция** — это **разделение** компонентов таким образом, чтобы отказ одного не влиял на другие. Изоляция **не добавляет избыточных элементов** — она **ограничивает распространение отказа**.

> **Оригинал:** *"Isolation - Systems are made from parts that are as physically and logically independent as possible. Failures in one part do not cascade into failures in an independent part."*
>
> **Перевод:** «Изоляция — системы состоят из частей, которые являются настолько физически и логически независимыми, насколько это возможно. Отказы в одной части не каскадируются в отказы независимой части.»

### Различие в марковской модели с hot-swap

| Аспект | **Резервирование** | **Изоляция** |
|:---|:---|:---|
| **Структура** | Параллельные блоки (1-out-of-n) | Разделение на независимые домены |
| **Переходы в марковской цепи** | Отказ → переход в состояние с меньшим числом элементов, **возврат через μ** | Отказ → **потеря домена**, возврат через DR/замену |
| **Восстановление** | Hot-swap возвращает систему в **предыдущее состояние** | Hot-swap **не восстанавливает** потерянный домен автоматически |
| **Влияние на MTBF** | MTBF **увеличивается** с числом резервных элементов | MTBF **не увеличивается** (но снижается вероятность полного отказа) |
| **Критерий отказа** | Отказ **всех** резервных элементов | Отказ **критического** компонента домена |

### Ключевое различие с учётом hot-swap

**В резервировании** hot-swap заменяет отказавший элемент, и система **возвращается в предыдущее состояние** с тем же числом работающих элементов. Марковская цепь имеет **обратные переходы** (μ), и система может **восстанавливаться** без потери функциональности.

**В изоляции** hot-swap **заменяет** отказавший компонент, но **не восстанавливает** потерянный домен автоматически. Если домен уже отказал (потеря данных или остановка), его восстановление требует **ручного вмешательства** — повторной конфигурации, перезапуска приложений и т.д. Марковская цепь **не имеет обратного перехода** из состояния отказа домена в работоспособное состояние без внешнего вмешательства.

> **Оригинал:** *"One speaks of fault isolation areas or fault containment units if the faults within such an area may be related but the faults in different areas are independent."*
>
> **Перевод:** «Говорят об областях изоляции отказов или блоках локализации отказов, если отказы внутри такой области могут быть связаны, но отказы в разных областях независимы.»

### Почему это важно для расчёта надёжности

В **резервировании** система может **деградировать**, но оставаться работоспособной, и hot-swap позволяет **вернуться** к полной конфигурации. MTBF **увеличивается** с числом резервных элементов.

В **изоляции** система **теряет** часть функциональности (домен) при отказе, и hot-swap **не восстанавливает** эту функциональность автоматически. MTBF **не увеличивается**, но **снижается вероятность полного отказа** системы, поскольку отказ одного домена не затрагивает другие.

> **Оригинал:** *"The suggested reliability estimation methods in this paper are the Reliability Block Diagram (RBD) and the Monte-Carlo Simulation methods."*
>
> **Перевод:** «Предлагаемые методы оценки надёжности в этой статье — диаграмма блоков надёжности (RBD) и метод Монте-Карло.»

В RBD **резервирование** выражается **параллельным** соединением блоков, а **изоляция** — **разделением** на независимые последовательные блоки, каждый из которых может отказать независимо.
...
| Вопрос | Ответ |
|:---|:---|
| **Марковская цепь для «хотя бы один CPU»** | 17 состояний (S16–S0), S0 — поглощающее состояние отказа |
| **Состояние отказа** | S0 (0 CPU) — полный отказ, потеря данных |
| **Резервирование vs изоляция** | Резервирование = параллельные блоки с обратными переходами; Изоляция = независимые домены без автоматического восстановления |
| **Влияние hot-swap** | В резервировании hot-swap возвращает систему в предыдущее состояние; в изоляции — заменяет компонент, но не восстанавливает домен |
| **Влияние на MTBF** | Резервирование увеличивает MTBF; изоляция не увеличивает, но снижает вероятность полного отказа |

Если нужно, могу детализировать расчёт стационарных вероятностей для этой марковской цепи или построить RBD для конкретной конфигурации M8000.

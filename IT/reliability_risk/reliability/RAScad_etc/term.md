...
Для построения Марковских моделей в системе с резервированием $N=2, K=1$ (два компонента, для работы нужен хотя бы один) введем стандартные базовые обозначения интенсивностей:

* $\lambda$ — интенсивность отказов одного компонента.
* $\mu$ — интенсивность ремонта одного компонента.

Для сценариев с прерыванием (Nontransparent) в Марковскую цепь добавляются переходные промежуточные состояния, так как процесс переключения или ввода в строй занимает время и временно лишает систему доступности. Введем для них:

* $\gamma$ — интенсивность автоматического переключения (восстановления) сервиса после сбоя.
* $\beta$ — интенсивность административных процедур по подготовке и согласованию окна обслуживания для ремонта.

------------------------------
## Сценарий 1: Transparent recovery, transparent repair
Идеальный случай. Переходы происходят мгновенно и без промежуточных задержек.

* Состояния:
* [2] — оба компонента исправны (Up).
   * [1] — один компонент отказал, сервис работает на втором (Up).
   * [0] — оба компонента отказали, система полностью лежит (Down).

## Схема Mermaid

graph LR
    2((2: Оба работают)) -- 2λ --> 1((1: Один отказал))
    1 -- λ --> 0((0: Полный отказ))
    1 -- μ --> 2
    0 -- μ --> 1
    
    style 2 fill:#d4edda,stroke:#28a745
    style 1 fill:#fff3cd,stroke:#ffc107
    style 0 fill:#f8d7da,stroke:#dc3545

## Дифференциальные уравнения (Колмогорова)
$$\begin{aligned} \frac{dP_2(t)}{dt} &= -2\lambda P_2(t) + \mu P_1(t) \\ \frac{dP_1(t)}{dt} &= 2\lambda P_2(t) - (\lambda + \mu) P_1(t) + \mu P_0(t) \\ \frac{dP_0(t)}{dt} &= \lambda P_1(t) - \mu P_0(t) \end{aligned}$$ 
Коэффициент готовности: $A = P_2 + P_1$
------------------------------
## Сценарий 2: Transparent recovery, nontransparent repair
Сбой скрыт, но после ремонта узел нельзя ввести в строй на лету. Появляется состояние ожидания планового окна обслуживания [1_Wait]. Пока узел ждет ввода в строй, система все еще уязвима.

* Состояния:
* [2] — оба компонента исправны (Up).
   * [1] — один отказал, работа на втором (Up).
   * [1_Wait] — физический ремонт завершен, ожидание окна ввода (Up).
   * [0] — оба отказали (Down).
...
graph LR
    ...
    style 2 fill:#d4edda,stroke:#28a745
    style 1 fill:#fff3cd,stroke:#ffc107
    style 1W fill:#e2e3e5,stroke:#6c757d
    style 0 fill:#f8d7da,stroke:#dc3545

## Дифференциальные уравнения
$$\begin{aligned} \frac{dP_2(t)}{dt} &= -2\lambda P_2(t) + \beta P_{1W}(t) \\ \frac{dP_1(t)}{dt} &= 2\lambda P_2(t) - (\lambda + \mu) P_1(t) + \mu P_0(t) \\ \frac{dP_{1W}(t)}{dt} &= \mu P_1(t) - (\lambda + \beta) P_{1W}(t) \\ \frac{dP_0(t)}{dt} &= \lambda P_1(t) + \lambda P_{1W}(t) - \mu P_0(t) \end{aligned}$$ 
Коэффициент готовности: $A = P_2 + P_1 + P_{1W}$
------------------------------
## Сценарий 3: Nontransparent recovery, transparent repair
Сбой вызывает падение сервиса, требуется время на переключение. Появляется состояние «переключения на резерв» [Rec]. В этом состоянии система временно недоступна для пользователя.

* Состояния:
* [2] — оба исправны (Up).
   * [Rec] — идет процесс переключения/восстановления (Down).
   * [1] — переключение завершено, работа на одном узле (Up).
   * [0] — оба отказали (Down).

## Схема Mermaid

graph LR
    2((2: Оба работают)) -- 2λ --> R((Rec: Переключение))
...
------------------------------
## Сценарий 4: Nontransparent recovery, nontransparent repair
Самый сложный случай. Задержка и падение доступности происходят как при движении «вниз» (авария и переключение [Rec]), так и при движении «вверх» (ожидание технического окна для ввода отремонтированного узла [1_Wait]).

* Состояния: См. граф ниже.
...
## Дифференциальные уравнения
$$\begin{aligned} \frac{dP_2(t)}{dt} &= -2\lambda P_2(t) + \beta P_{1W}(t) \\ \frac{dP_{Rec}(t)}{dt} &= 2\lambda P_2(t) - \gamma P_{Rec}(t) \\ \frac{dP_1(t)}{dt} &= \gamma P_{Rec}(t) - (\lambda + \mu) P_1(t) + \mu P_0(t) \\ \frac{dP_{1W}(t)}{dt} &= \mu P_1(t) - (\lambda + \beta) P_{1W}(t) \\ \frac{dP_0(t)}{dt} &= \lambda P_1(t) + \lambda P_{1W}(t) - \mu P_0(t) \end{aligned}$$ 
Коэффициент готовности: $A = P_2 + P_1 + P_{1W}$
------------------------------
Если вам необходимо, мы можем составить матрицу переходов (генератор $Q$) для любого из этих сценариев, чтобы рассчитать финальные стационарные вероятности для RAScad. Какой сценарий рассчитать в первую очередь?

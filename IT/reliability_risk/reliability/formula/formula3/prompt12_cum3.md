...
Расширь существующую «Восьмипозиционную модель» двухузлового кластера для трехузлового кластера с одной ремонтной бригадой.
...
## 2. Переходы для трехузловой модели
...
**Важно:** Из S_latent возможен только один переход в S0_fail с интенсивностью θ. На графе это изображается одной дугой, так как в случае скрытого отказа неважно, из какого состояния (S3, S2 или S1) произошел переход в S_latent — требуется единая процедура выявления отказа внешними средствами.
...
- Укажи общее число состояний модели.

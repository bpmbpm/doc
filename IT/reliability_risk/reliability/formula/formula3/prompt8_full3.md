...
Считаем, что кластер в S1_tf, S2_tf находится в состоянии отказа (на время перезагрузки после сбоя).
...
- S_failback — возврат восстановленного узла, транзитный (промежуточный) переход S1 → S2.
...
7. Рассчитай:  
7.1. Стационарный коэффициент готовности (формулу представь через λ и μ)  
...
- Направление графа: слева направо (LR). Причем по мере деградации состояний, например, оба узла работают -> отказал один узел -> отказали оба узла;

## step
- [Outline: Делаем свой личный VPN от Google за 5$ в месяц (и за 1€ для продвинутых)](https://habr.com/ru/articles/358828/)
  - OpenVPN
## hosting
- deephost.pw Сергей
  - https://ru.hostings.info/deephost-pw.html ; https://vps.today/companies/deephost-pw Просят телефон при регистрации...в топку
### hosting 2
- https://poiskvps.ru/companies/deephost-pw/reviews
- https://phpbbx.ru/hosting.html ; VPS \ VDS https://phpbbx.ru/topic52.html
- https://noisycake.ru/articles/vpn_server/ Свой VPN для самых маленьких | 3X-UI + VLESS
### yandex 
 - https://qna.habr.com/q/1389866 яндекс облако - это не vps, а PaaS типа Azure или AWS, с возможностью делать всё через API, поминутной тарификацией, кучей managed сервисов и т.д.

### WL
- https://github.com/zieng2/wl

### v2RayTun
- [Настройка VLESS и Reality](https://noisycake.ru/articles/vpn_server/#%D0%BD%D0%B0%D1%81%D1%82%D1%80%D0%BE%D0%B9%D0%BA%D0%B0-vless-%D0%B8-reality)
- v2RayTun_Setup.exe

### habr
- https://www.pvsm.ru/cat/obhod-blokirovok
- [Мой VPN пережил белые списки. Архитектура из 4 уровней за 265₽ в месяц](https://habr.com/ru/articles/1021160/) ; https://www.pvsm.ru/vpn/449328
- [В попытках разобраться с VPN зачем то написал Bash-костыль и немного проализировал обстановку 2026 году](https://habr.com/ru/articles/1046263/) Есть ли сайты, где копируется не только статья с хабра, но и коменты к ней? Например, помню, что в https://www.pvsm.ru/vpn/449328 в коментах хабра был хороший анализ трехсерверного варианта (цепочка из трех), но теперь коменты не доступны. Так и с этой будет.  

## 3
- [Прячем метаданные в мессенджере: 2-hop onion-lite поверх обычных VLESS + Reality relay, и почему это почти бесплатно](https://habr.com/ru/articles/1047920/)

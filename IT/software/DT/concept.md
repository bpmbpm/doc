...
Повтор: для DT важен не код, который исполнит точно то же, что и PT, а понятная модель, т.е. важнее, не что будет сделано, а как будет сделано: "опрозрачить" логику работы (организации, программы  т.п.) и ее запустить в симуляторе с новым набором вводных (если речь про предсказания).  

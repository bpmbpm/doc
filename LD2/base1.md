...
:id1-predicateName-id2 - это группа компонент, в которой выделяется первый компонент со свойством *protocol = jdbc* и второй со свойством *protocol = DirectLink*. Задание id дочерним компонентам также должно быть задано правилом, например, через разделитель ".".   

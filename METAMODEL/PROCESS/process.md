## Что такое процесс?
### Требуется
- МетаМодель процесса, а также формализм ее на каком-либо языке (DSL)
- математизация сущности "процесс" (математический формализм)
- маппинг этого понятия на иные (синонимы с разной "окраской") в разных нотациях (Activity  - BPMN, function - EPC и т.п.). 
### Как вариант
- наиболее точный и наглядный формализм процесса в ЕРС: как сочетание workflow (событийные входы управляют запуском функций \ операций) и docflow (в обхем случае вещевой \ вещественный \ документарный вход \ выход). Наглядно: [ВРМ. Смарт-инструменты «Таблица -> Схема» ...](https://habr.com/ru/articles/810851/), если про шаблоны workflow, то http://www.workflowpatterns.com/ - можно считать, что в ЕРС стрелки workflow - это событийные входы \ выходы, в стрелки docflow (слева от функции) - это вещественные входы \ выходы. Роли и инструменты (слева от функции) - это ресурсы \ обеспечение. Сам алгоритм - это и есть сама фуункция (x2, x3 и т.п.), т.е. "алгоритмическое обеспечене" функции (см. старые ГОСТ). Если в уравнении учесть все составляющие, например, по аналогии с комплексным числом (i - например, событийный вход), то можно вывести "полную формулу процесса".     

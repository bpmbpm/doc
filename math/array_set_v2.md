...
Ниже представлен **полный** набор формализмов с исправленным форматированием: пустые строки до и после каждого блока `$$`, корректное отображение матриц (проверенные варианты) и таблица с краткими описаниями. Все подходы, перечисленные ранее, сохранены.
...
| Смешанный (алгебра)    | $\text{out}_i = \Phi_i(\sum_j \alpha_{ij} \text{in}_j)$ |
...
### 12. Варианты отображения матрицы на GitHub

Для гарантированного отображения матриц в Markdown с поддержкой MathJax (GitHub) используйте одно из следующих окружений. Каждый пример приведён в отдельном блоке `$$` с пустыми строками.
...
### 13. Сохранение результата в PDF
